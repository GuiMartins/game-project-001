...
func _ready() -> void:
	...
	_door_mesh.position = Vector3(-(SIZE.x + DOOR_SIZE.x) * 0.5, -0.1, 0.2)
	_door_mesh.visible = false
	add_child(_door_mesh)

	_door_shape = Greybox.box_shape(DOOR_SIZE)
	_door_shape.position = _door_mesh.position
	_door_shape.disabled = true
	add_child(_door_shape)


func setup(a_track: RoadTrack, a_offset: float, a_lateral: float, seed_value: int) -> void:
	track = a_track
	offset = a_offset
	lateral = a_lateral
	...
	speed = _rng.randf_range(0.0, 7.0)
...
func recycle(a_offset: float, a_lateral: float) -> void:
	offset = a_offset
	lateral = a_lateral
	_target_lateral = a_lateral
	speed = _rng.randf_range(0.0, 7.0)
